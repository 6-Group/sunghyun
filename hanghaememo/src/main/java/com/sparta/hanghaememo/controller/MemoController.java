...
public class MemoController {
        ...
        @PostMapping("/api/memos") // index파일 writePost에서 Post 방식 url "/api/memos" 부분을 받는 서버부분을 개발
        public Memo createMemo(@RequestBody MemoRequestDto requestDto) {  //Memo를 바로 반환할 것이기에 Memo라고 해줍니다.
                ...
                return memoService.createMemo(requestDto); //memoService의 creatMemo라는 연결되는 부분을 method로 만들어주고  parameter값으로  클라이언트에서 가져 온 requestDto 안에 들어있는 값들을 사용해야 돼서 만들고 createMemo함수를 만들러 간다.memoService로 고고
        }

        @GetMapping("api/memos") //index파일 getMessages()에서 "GET"방식 "/api/memos"을 가져온 것임.
        public List<Memo> getMemos() { //List형식으로 반환타입 해줌, Client에서 전달해주는 데이터가 없기에 parameter는 넣을 필요가 없다.
